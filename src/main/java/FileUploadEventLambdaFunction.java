 ...
 */
public class FileUploadEventLambdaFunction implements RequestHandler<Order,String> {
    public FileUploadEventLambdaFunction() {
    }
      ...
      public String handleRequest(Order event, Context context) {
        try {
            ...
            String receiptContent = String.format("OrderID: %s\nAmount: $%.2f\nItem: %s",
                    event.getOrderId(), event.getAmount(), event.getItem());
            String key = "receipts/" + event.getOrderId() + ".txt";

            // Upload the receipt to S3
            uploadReceiptToS3(bucketName, key, receiptContent);

            context.getLogger().log("Successfully processed order " + event.getOrderId() +
